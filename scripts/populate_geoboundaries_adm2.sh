...
# Populates the adm2_boundaries table from the geoBoundaries CGAZ ADM2 GeoJSON.
#
...
#   2. A psql step transfers from staging → adm2_boundaries with explicit column
...
echo "Transferring ADM2 data to adm2_boundaries..."

docker exec -i "$DB_CONTAINER" psql -U "$DB_USER" -d "$DB_NAME" << SQL
TRUNCATE TABLE adm2_boundaries;

INSERT INTO adm2_boundaries (shape_id, shape_name, country, geom)
SELECT
    shapeid     AS shape_id,
...
DROP TABLE IF EXISTS staging_adm2;

SELECT COUNT(*) AS total_adm2 FROM adm2_boundaries;
SQL

echo "adm2_boundaries table populated."
